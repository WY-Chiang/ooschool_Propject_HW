...
# ===================== 1. 讀取所有 CSV =====================
csv_files = glob.glob("ML_Quarterly_Dataset_JNJ.csv")  # 確認路徑正確
all_dfs = []
for f in csv_files:
...
for name, model in models.items():
    ...
    # --- y_test vs y_pred 對比圖 ---
    plt.figure(figsize=(12,6))
    plt.plot(y_test.values[:100], label='Actual (y_test)', marker='o', alpha=0.7)
    plt.plot(y_pred[:100], label='Predicted (y_pred)', marker='x', alpha=0.7)
    # plt.title(f"{name} - 前100筆 預測 vs 實際")
    # plt.xlabel("樣本順序 (時間)")
    # plt.ylabel("是否上漲 (1=上漲, 0=下跌)")
    plt.legend()
    plt.grid(True)
    plt.show()
